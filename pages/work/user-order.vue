			...
			async contactOrderParty(order) {
				...
				try {
					...
					console.log('📋 订单详情:', {
						订单类型: order.type,
						客户ID: order.userId,
						承接方ID: order.contractorId,
						当前用户ID: this.userInfo.userId
					});
					...
					if (isDesignerOrSupervisor) {
						...
						// 防止联系自己
						if (String(targetUserId) === String(this.userInfo.userId)) {
							uni.showToast({
								title: '不能联系自己',
								icon: 'none',
								duration: 2000
