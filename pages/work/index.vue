				...
				<view class="order-footer">
					...
					<view class="order-actions">
						...
						<template v-else-if="order.status === 2">
							...
							<!-- 用户端显示评价按钮，根据是否有评价决定是否显示 -->
							<button v-if="isCustomer && !order.hasReview" class="btn primary" @click="goToReview(order.orderId)">
								评价订单
							</button>
							<!-- 如果已有评价，显示已评价状态 -->
							<text v-if="isCustomer && order.hasReview" class="status-text">
								已评价
							</text>
						</template>
					</view>
				...
				async loadOrderList() {
					...
					try {
						...
						for (const order of list) {
							...
							// 检查订单是否有评价（只在已完成状态且是用户身份时检查）
							let hasReview = false
							if (order.status === 2 && this.isCustomer) {
								try {
